...
public class PlayerHeadsMenu {
    ...
    private static List<HeadsItem> headsItems = new ArrayList<HeadsItem>();
    private static HashMap<String,List<HeadsItem>> playerheadmenus = new HashMap<String, List<HeadsItem>>();
    ...
    public static void loadHeadItems() {
        ...
        if(!config.contains("animals")){
            ...
            config.set("animals.lore", Arrays.asList(new String[]{"Click to open", "animals head menu"}));
        ...
        for(HeadsItem headsItem:headsItems){
            ...
            List<HeadsItem> list = new ArrayList<HeadsItem>();
            if(!config.contains("example")){
                ...
                config.set("example.lore", Arrays.asList(new String[]{ChatFormatter.formatMessage("&7Click to select")}));
