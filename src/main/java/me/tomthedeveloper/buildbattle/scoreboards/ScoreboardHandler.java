...
public class ScoreboardHandler {
    ...
    public ScoreboardHandler(BuildInstance buildInstance){
        ...
        scoreboardLoader = new ScoreboardLoader( buildInstance.getPlugin());
    }
    ...
    private String format(String string, Player player){
        ...
        if(ConfigPreferences.isVaultEnabled()){
            string.replaceAll("%MONEY%", Double.toString(BuildBattle.getEcon().getBalance(player.getName())));
            string.replaceAll("%GROUP%", BuildBattle.getPerms().getPrimaryGroup(player));
        }
        return string;
    }
